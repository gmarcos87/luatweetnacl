...
print("box() -- ")
print("generate a key pair for Alice")
local apk, ask = nacl.box_keypair()  -- public key, secret key

print("generate a key pair for Bob")
local bpk, bsk = nacl.box_keypair()  -- public key, secret key

print("Alice encrypts for Bob with Bob public key and her secret key")
et = nacl.box(pt, n, bpk, ask) 

print("the encrypted text include a Poly1305 MAC (16 bytes)")
assert(#et == #pt + 16)

print("Bob decrypts the message with  Alice public key and his secret key")
dt = nacl.box_open(et, n, apk, bsk) -- bob decrypts
assert(dt == pt)

print("secretbox() -- authenticated encryption (Salsa20 and Poly1305 MAC)")

print("encrypt the plain text")
et = nacl.secretbox(pt, n, k)

print("decrypt the encrypted text (the Poly1305 MAC is checked)")
dt, msg = nacl.secretbox_open(et, n, k)
assert(dt == pt)

print("check secretbox() works with an empty text")
local ez = nacl.secretbox("", n, k)
local dz, msg = nacl.secretbox_open(ez, n, k)
assert(#ez == 16 and #dz == 0)

print("stream, stream_xor -- this exposes the raw Salsa20 algorithm")
local s3 = nacl.stream(3, n, k)
local e3 = nacl.stream_xor("\0\0\0", n, k)
assert(s3 == e3)

print("poly1305 MAC 	(one of the rfc 7539 test vectors)")
...
print("hash a.k.a. sha512 - check the simplest test vector")
local s, h
s = "abc"
